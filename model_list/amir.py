...
class SparseDispatcher(object):
    ...
    def __init__(self, num_experts, gates):
        ...
        # Calculate num samples that each experts gets
...
class MoE(nn.Module):
    ...
    def __init__(self, input_size, output_size, mlp_ratio, num_experts, noisy_gating=True, use_experts=2):
        ...
        self.w_noise = nn.Parameter(torch.randn(2 * input_size, num_experts), requires_grad=True)

        self.softplus = nn.Softplus()
        self.softmax = nn.Softmax(dim=1)
        self.register_buffer("mean", torch.tensor([0.0]))
        self.register_buffer("std", torch.tensor([1.0]))
        assert self.k <= self.num_experts, "k should be no more than num_experts"
    ...
    def noisy_top_k_gating(self, x, train, noise_epsilon=1e-2):
        ...
        top_k_gates = top_k_gates.to(dtype=logits.dtype)  # ✅ dtype 맞추기
    ...
    def _prob_in_top_k(self, clean_values, noisy_values, noise_stddev, noisy_top_values):
        ...
        threshold_position_if_in = torch.arange(batch, device=clean_values.device) * m + self.k
        threshold_if_in = torch.unsqueeze(torch.gather(top_values_flat, 0, threshold_position_if_in), 1)
        is_in = torch.gt(noisy_values, threshold_if_in)

        threshold_position_if_out = threshold_position_if_in - 1
        threshold_if_out = torch.unsqueeze(torch.gather(top_values_flat, 0, threshold_position_if_out), 1)
...
class RIN(nn.Module):
    def __init__(self,
        ...
        self.CondNet = nn.Sequential(
            nn.Conv2d(in_dim, hidden_dim, 3, 3), nn.LeakyReLU(0.1, inplace=True),
            nn.Conv2d(hidden_dim, hidden_dim, 3, 3), nn.LeakyReLU(0.1, inplace=True),
            nn.Conv2d(hidden_dim, hidden_dim, 1), nn.LeakyReLU(0.1, inplace=True),
            nn.Conv2d(hidden_dim, hidden_dim, 1), nn.LeakyReLU(0.1, inplace=True),
...
class AMIR(nn.Module):
    ...
    def forward(self, data_batch):
        ...
        self.loss_importance = loss_importance + loss_tmp
    ...
    def _calculate_loss(self, prediction, ground_truth):
        ...
        return reconstruction_loss + 0.001 * self.loss_importance